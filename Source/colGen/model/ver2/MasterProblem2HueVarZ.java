...
public class MasterProblem2HueVarZ {
			...
			public MasterProblem2HueVarZ(int coreCount, List<FuncPt> func_list, Map<Integer, Integer> funcCountMax, List<Integer> scUsed, 
					...
					ArrayList<PricingProblem2> pPList, Map<Integer,Integer> replicaPerVNF, int kNodeCount, Set<HuerVarZ> prevCols) throws Exception{		
...
//			    }
			    ...
			    for(int sd_count=0; sd_count < pair_list.size(); sd_count++){
			    	...
			    	//create the range to be considered
			    	String constraint = "cstr" + cstrIndicator + "_" + cstrNum + "_sd" + sd_count;
			     ...
			     for(TrafficNodes tn : pair_list){
			    	 int sd_index = pair_list.indexOf(tn);
			    	...
			    	for(BaseVertex nfvi_node : temp_nfvi_nodes){
			    		...
			    		//create the range to be considered
				    	String constraint = "cstr" + cstrIndicator + "_" + cstrNum + "_sd" + sd_index;
			    		IloRange rng = this.master_problem.addRange(-Double.MAX_VALUE, 0.0,  constraint);
			    		cstrNum++;			    		
...
//				    	     IloNumVar tempVar = this.master_problem.numVar(0.0, Double.MAX_VALUE, slackVar);
			     ...
			     for(int scID : scUsed){
			    	 ...
			    	 if(prevCols.isEmpty()){
			    	    for(HuerVarZ config : configsPerSC.get(scID)){
				    		...
				    		this.var_z_gamma.put(config, this.master_problem.column( this.BwUsed, coeffZObj));	
			//	    		System.out.println("Coefficient for VarZGamma in this.BwUsed : " + entry.getKey().flow_traffic*link_included );
			    	    }
			    	 }else{
			    		 for(HuerVarZ config : prevCols){
			    	    	double coeffZObj = 0.0;
			    	    	for(Pp2VarDelta varD : config.DeltaVarSet){
			    	    		coeffZObj += varD.sd.flow_traffic*config.BVarSet.size();
			    	    	}				    		
					   ...
					   for(int scID : scUsed){
					    	 ...
					    	 for(Map.Entry<MpVarZ, IloColumn> entry : this.var_z_gamma.entrySet()){
					    		...
					    		if(config.sCiD == scID){				    			
				    				...
				    				//add the coefficient for the var_z_gamma variable
				    				this.var_z_gamma.put(entry.getKey(),entry.getValue().and(this.master_problem.column(this.core_capacity_constraint.get(vnf_node_index), coeffCore)));		    				
					    		}
					    	 }	
					   }
				   }
					 	 ...
					 	 for(int scID : scUsed){
						   ...
						   for(Map.Entry<MpVarZ, IloColumn> entry : this.var_z_gamma.entrySet()){
							   	...
							   	if(config.sCiD == scID){
				    			     ...
				    			     this.var_z_gamma.put(entry.getKey(),entry.getValue().and(this.master_problem.column(this.flow_capacity_constraint.get(link_counter),link_included*totalTrafficOverConfig)));
							   	}
							}						   	
						 }
				    	//increment the link counter				   
			...
			if(InputConstants.latencyCstr){
				for(Map.Entry<MpCstr7and11,IloRange> entryCstr : this.flow_latency_constraint.entrySet()){
					...
					HuerVarZ config = configPerSD.get(tn).conFiguration;			   
    				double coeff = 0.0;
    				...
    				//add the coefficient to var_z_gamma for the link constraints
    				IloColumn col = this.var_z_gamma.get(config);
			        this.var_z_gamma.put(config,col.and(this.master_problem.column(entryCstr.getValue(),coeff)));							
				}
			}
			//one path per c & sd constraint (9)				
			for(Map.Entry<MpCstr7and11,IloRange> entryCstr : this.path_per_c_sd_contraint.entrySet()){				
			   	...
			   	HuerVarZ config = configPerSD.get(sd).conFiguration;
			   	IloColumn col = this.var_z_gamma.get(config);
			   	//add the coefficient for var_z_gamma to the constraint
	    		this.var_z_gamma.put(config,col.and(this.master_problem.column(entryCstr.getValue(),1)));			   		   
			}  
			//source outgoing link constraint (10)				
...
//  		    	String zName = "Z_SC"+ entry.getKey().sCiD + entry.getKey().configDsc;
  		    	String zName = "Z_SC"+ entry.getKey().sCiD + entry.getKey().configDsc + "_(" + sdPairs + ")" ; 
...
//			   System.out.println("Node: " + node.get_id() + " = " + this.vnf_location_constraint_2.get(node).toString());
		   ...
		   for(Map.Entry<MpVarH, IloColumn> entry : var_h_v.entrySet()){
		  	 ...
		  	 this.usedVarH.put(entry.getKey(),this.master_problem.numVar(entry.getValue(), 0, 1, hName));
		   }
...
//			   			    	System.out.println("Link : ( " + s_vrt.get_id() + " , " + non_nfvi_vrt.get_id() + " )");  				   			    
			...
			public MasterProblem2HueVarZ(int coreCount, List<FuncPt> func_list, Map<Integer, Integer> funcCountMax, List<Integer> scUsed, 
					...
					Map<TrafficNodes,SdDetails> configPerSD, Graph g, ArrayList<BaseVertex> nodesNFVI, ArrayList<BaseVertex> nfv_nodes, 
					Map<Integer, ServiceChain> ChainSet, List<TrafficNodes> pair_list, ArrayList<BaseVertex> vertex_list_without_nfvi_nodes,
					ArrayList<PricingProblem2> pPList, Map<Integer,Integer> replicaPerVNF, int kNodeCount) throws Exception{		
