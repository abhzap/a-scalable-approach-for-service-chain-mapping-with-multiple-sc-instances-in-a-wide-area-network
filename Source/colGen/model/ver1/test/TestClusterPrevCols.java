...
public class TestClusterPrevCols {
	...
	public static void runTest() throws Exception{
	    ...
	    //keep track of the columns used across various instances
	    Map<Integer, HashSet<HuerVarZ>> generatedColumns = new HashMap<Integer, HashSet<HuerVarZ>>();
	    ...
	    for(int kNodeCount=g._vertex_list.size(); kNodeCount>0; kNodeCount--){
		    ...
		    for(int numOfClusters=1;numOfClusters<=ClusterHeuristic.formClustersWhenAllSdPairs(g).size();numOfClusters++){	  
				...
				CG.runCG(coreCstr,capCstr,cpuCoreCount,linkCapacity,g, ChainSet, pair_list, scUsed, vnf_list, func_list, serviceChainTN, nfv_nodes, 
						  nodesNFVI, vertex_list_without_nfvi_nodes, scCopies, scCopyToSC, configsPerSC, configPerSD, CountMaxVNF, replicaPerVNF,kNodeCount, generatedColumns);
