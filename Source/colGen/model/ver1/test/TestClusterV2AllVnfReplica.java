...
public class TestClusterV2AllVnfReplica {
...
//		for(int kNodeCount=1; kNodeCount<=numOfNfvNodes; kNodeCount++){
			...
			String filename = "VnfReplica_" + InputConstants.NETWORK_FILE_NAME.substring(0, InputConstants.NETWORK_FILE_NAME.lastIndexOf(".")) + ".txt";
...
//    	    for(int numOfClusters=1;numOfClusters<=maxClusterCount;numOfClusters++){
				...
				//print the configurations for each SC
			  	...
			  	for(int replicaCount=g._vertex_list.size(); replicaCount>=1; replicaCount--){			  	
			  		//int replicaCount=g._vertex_list.size();			  		   		    
	    		    //replica constraint per VNF
	    		    Map<Integer,Integer> replicaPerVNF = new HashMap<Integer,Integer>();
	    		    for(int vnfID : CountMaxVNF.keySet()){
					...
					//write to a file
			  	  	bw.write(replicaCount + "\t" + CG.rmpIlpValue + "\t" + preProcFunctions.totalBwUsedOnShortestPaths(sdpaths, pair_list) + "\t" + 
			  	  			CG.vnfPlacementNodes.size() + "\t" + CG.vnfPlacementNodes.toString() + "\t" +
